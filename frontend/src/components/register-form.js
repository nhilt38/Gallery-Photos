...
export default function RegisterForm() {
  ...
  const handleLogin = async (e) => {
    e.preventDefault();
    let success = await register(dispatch, { email, password, name });
    if (success) {
      navigate("/login");
    } else {
      setTimeout(() => {
  ...
  return (
    ...
    >
      <Stack spacing="8">
        <Stack spacing="6">
          ...
          <Stack spacing={{ base: "2", md: "3" }} textAlign="center">
            ...
            <Text color="fg.muted">
              Did you have an account? <Link href="/login">Sign in</Link>
            </Text>
          </Stack>
        </Stack>
        <Box
      ...
      {showAlert && (
        <Alert status="error">
          <AlertIcon />
          Register failed, email is already registered
        </Alert>
      )}
    </Container>
  );
...
//   const handleLogin = async (e) => {
//     e.preventDefault();
//     await register(dispatch, {email, password, name});
//     navigate('/login');
//   };
